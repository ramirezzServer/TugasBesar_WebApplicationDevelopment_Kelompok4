                        ...
                        <form action="{{ route('admin.sopir.update', $sopir->id) }}" method="POST"
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="nama_sopir" class="form-control" required
                                    value="{{ old('nama_sopir', $sopir->nama_sopir) }}" placeholder="Masukkan nama lengkap">
                            </div>

                            <div class="mb-3">
                                <label class="form-label">No Telepon</label>
                                <input type="number" name="notelp_sopir" class="form-control" required
                                    value="{{ old('notelp_sopir', $sopir->notelp_sopir) }}" placeholder="08...">
                            </div>

                            <div class="mb-3">
                                <label class="form-label">Alamat</label>
                                <textarea name="alamat" class="form-control" rows="3" required placeholder="Masukkan alamat lengkap">{{ old('alamat', $sopir->alamat) }}</textarea>
                            </div>

                            <div class="mb-3">
                                <label class="form-label">Email</label>
                                <input type="email" name="email_sopir" class="form-control" required
                                    value="{{ old('email_sopir', $sopir->email_sopir) }}" placeholder="email@contoh.com">
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                @if ($sopir->foto)
                                    <div class="mb-2">
                                        <img src="{{ asset('storage/' . $sopir->foto) }}" alt="Foto Lama" width="100"
                                            class="img-thumbnail">
                                    </div>
                                @else
